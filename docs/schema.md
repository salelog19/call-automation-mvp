...
Также на уровне SQL стоит проверять, что `tracking_number_id` принадлежит тому же `project_id`, что и сам `visit`.
...
- один `visit` может иметь 0, 1 или несколько связанных `calls`

Ограничения целостности, которые полезно держать уже в MVP:

- `visit.tracking_number_id` должен ссылаться только на номер из того же `project`
- `call.tracking_number_id`, если он заполнен, должен ссылаться только на номер из того же `project`
- `call.visit_id`, если он заполнен, должен ссылаться только на `visit` из того же `project` и с тем же `tracking_number_id`
